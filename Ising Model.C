...
// A message variable is passed to this section, which is used to then call subsequent functions
...
void IsingModel(int iMsg)
{
	...
	// if message = 3, call the Metropolis Montecarlo function to loop over lattice to change spins
	else if (iMsg == 3)
	{
...

{
	...
	for (i=0;i<size;i++) 
	{
		for (j=0;j<size;j++) 
		{
			...
			iSpinSum = matSpin[i][iPosCol] + matSpin[iPosRow][j] + matSpin[i][iNegCol] + matSpin[iNegRow][j];
